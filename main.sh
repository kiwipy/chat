...
function send(){
	...
	if [ ! -z "$SAY" ];then
		for word in $SAY;do
			if [ $(grep -F "$word" /usr/share/stim/emoji ) ];then
				# Add emoji if pattern match
				SENTENCE+=("$(grep -F "$word" /usr/share/stim/emoji | sed 's/.*=//')")
			else
				SENTENCE+=("$word")
			fi
		done
...
function refresh(){
	...
	for i in $(cat $SERVER/$ROOM/USER_LIST);do
		if [ "$(grep $i $SERVER/$ROOM/USER_LIST | sed 's/.*=//')" == "1" ];then
			((count+=1))
		fi
	done
	HEADER="Chatroom: $( echo $ROOM | sed 's:.*/::') | Online: $count"
	HEADER_LEN=${#HEADER}
	...
	while true;do
		...
		case $CMD in
			...
			e)
				...
				repeat $WIDTH "_"
				count="0"
				for i in $(cat /usr/share/stim/emoji);do
					if [ "$count" == $((WIDTH/8)) ];then
						echo ""
						count="0"
					else
						echo -ne "$(grep $i /usr/share/stim/emoji | sed 's/.*=//') $(grep $i /usr/share/stim/emoji | sed 's/=.*//')\t"
						((count+=1))
					fi
				done
				echo -e "\n"
